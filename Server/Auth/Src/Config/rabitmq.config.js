...
export const connectRabbitMQ = async (exchangeName) => {
  try {
    ...
    logger.error(`Error connecting to RabbitMQ or asserting exchange "${exchangeName}":`, error);
  }
};
...
export const publishEvent = async (exchange,routingKey, message) => {
  try {
    ...
    logger.info(`Message published with exchange name "${exchange}" and routing key "${routingKey}"`);
  } catch (error) {
...
export const consumeEvent = async(exchange,routingKey,callback)=>{
  try {
    ...
    logger.info(`Message consume with exchange name "${exchange}" and routing key "${routingKey}"`);
  } catch (error) {
