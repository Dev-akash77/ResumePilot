...
const Ats = () => {
  ...
  const handleAnalyze = () => {
    ...
    // Call the hook function instead of setTimeout
    analyzeResume(resumeId, jobRole);
  };
      ...
      <div className="max-w-7xl mx-auto h-full">
        ...
        <div className="flex flex-col gap-1 mb-8">
          <h2 className="text-3xl font-bold text-gray-900 tracking-tight flex items-center gap-2">
            AI Resume Reviewer{" "}
            <LuSparkles className="text-blue-500" size={24} />
          </h2>
          <p className="text-gray-500 text-lg">
            Get a detailed ATS breakdown and scoring based on your target job
            role.
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
                ...
                <div className="relative w-32 h-32 flex items-center justify-center mb-4">
                  <svg className="w-full h-full transform -rotate-90">
                    ...
                    <circle
                      ...
                      strokeDasharray={351}
                      strokeDashoffset={
                        351 - (351 * analysisResult.score) / 100
                      }
                      className={`${getScoreColor(analysisResult.score)} transition-all duration-1000 ease-out`}
                    />
                  </svg>
                  <span
                    className={`absolute text-4xl font-bold ${getScoreTextColor(analysisResult.score)}`}
                  >
              ...
              <div className="lg:col-span-2 bg-white rounded-2xl p-6 shadow-sm border border-gray-200">
                ...
                <div className="space-y-6">
                  {/* Safely access breakdown properties */}
                  <ProgressBar
                    label="Keywords & Skills"
                    score={analysisResult.breakdown?.keywords || 0}
                  />
                  <ProgressBar
                    label="Formatting & Structure"
                    score={analysisResult.breakdown?.formatting || 0}
                  />
                  <ProgressBar
                    label="Impact & Metrics"
                    score={analysisResult.breakdown?.impact || 0}
                  />
                  <ProgressBar
                    label="Job Relevance"
                    score={analysisResult.breakdown?.relevance || 0}
                  />
                </div>
              </div>
            </div>
