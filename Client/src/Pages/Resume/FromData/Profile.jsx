...
const Profile = () => {
  ...
  const { generateSummary, loading } = useResumeSummary();

  const handleGenerateSummary = async () => {
    const text = await generateSummary(
      "Full Stack Developer",
      summaryDataRead?.data,
    );

    if (text) {
      dispatch(summaryChange({ name: "summary", value: text }));
    }
  };
      ...
      <div className="p-8">
        ...
        <div className="mb-8">
          <FromHeaders
            ...
            des={
              "Write a short summary of your career to catch recruiter's attention."
            }
          />
        </div>
        ...
        <form className="w-full flex flex-col gap-6">
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            ...
            <button
              ...
              disabled={loading}
              className="group relative inline-flex items-center justify-center gap-2 px-4 py-2 text-sm font-medium text-purple-700 bg-purple-50 border border-purple-100 rounded-lg hover:bg-purple-100 hover:border-purple-200 transition-all duration-300 shadow-sm hover:shadow-md active:scale-95 disabled:opacity-70"
            >
              {loading ? (
                <>
